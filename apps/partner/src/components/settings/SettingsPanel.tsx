...
export function SettingsPanel({ user, banks }: Props) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 lg:gap-12">
          ...
          return (
            <button
              ...
              className={[
                ...
                isActive
                  ? 'bg-(--color-surface) border-(--color-border) text-(--color-text) font-bold'
                  : 'font-medium text-zinc-400 hover:text-(--color-text) hover:bg-zinc-800/80 border-transparent',
              ].join(' ')}
            >
      ...
      {/* Content card */}
      <div className="flex-1 bg-(--color-surface) border border-(--color-border) rounded-xl p-6 sm:p-10">
