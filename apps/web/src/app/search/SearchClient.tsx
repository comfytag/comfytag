...
export function SearchClient({
  ...
  const [seasonal, setSeasonal] = useState<SeasonalConfig | null>(initialSeasonal)
      ...
      try {
        ...
        const mapped: Suggestion[] = raw
          ...
          .map((item) => ({
            ...
            href:
              item.type === 'event'
                ? `/events/${item.slug ?? item._id}`
                : item.type === 'organizer'
                  ? `/organizer/${item.slug ?? item._id}`
  ...
  async function handleNotifyMe(e: React.FormEvent) {
    ...
    try {
      await api.post('/alerts', { query, email })
  ...
  const filtered = results.filter((e) => {
    ...
    if (areaFilter !== 'all') {
      ...
      if (!(e.address ?? '').toLowerCase().includes(area) && !(e.venue ?? '').toLowerCase().includes(area))
        return false
    }
    return true
  })
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '24px' }}>
              ...
              {PRICE_PILLS.slice(1).map((p) => (
                <FilterPill
                  ...
                  onClick={() => setPriceFilter(p.key === priceFilter ? 'all' : p.key)}
                />
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
              {ILORIN_AREAS.map((a) => (
                <FilterPill
                  ...
                  onClick={() => setAreaFilter(a.key === areaFilter ? 'all' : (a.key as AreaFilter))}
                />
              ))}
            </div>
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        cursor: 'pointer',
                        opacity: notifyMe.loading ? 0.7 : 1,
                      }}
                    >
