...
const PARTNER_LINKS = [
  { label: 'Partner Suite', href: process.env.NEXT_PUBLIC_PARTNER_URL ?? 'http://localhost:3001', comingSoon: true },
          ...
          <div className="md:col-span-2">
            ...
            <ul className="space-y-4 text-(--color-text-muted) text-sm">
              {PARTNER_LINKS.map((item) =>
                item.comingSoon ? (
                  <li key={item.href}>
                    <span aria-disabled="true" className="flex items-center gap-2 opacity-60 cursor-not-allowed select-none">
                      {item.label}
                      <span className="text-[9px] uppercase tracking-wide font-bold bg-(--color-surface) px-1.5 py-0.5 rounded-full">Soon</span>
                    </span>
                  </li>
                ) : (
                  <li key={item.href}>
