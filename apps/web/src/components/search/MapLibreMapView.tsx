...
// Satellite tile style (ESRI World Imagery — free, no API key)
...
export function MapLibreMapView({ events, onEventSelect, onClose, selectedState }: MapViewProps) {
  ...
  // Fly-to animation when state filter changes
  useEffect(() => {
    if (!selectedState || !mapRef.current) return
    const coords = NIGERIA_STATE_COORDS[selectedState]
    if (!coords) return
    mapRef.current.flyTo({ center: [coords[0], coords[1]], zoom: 8, duration: 1000 })
  }, [selectedState])
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <Map
        ...
        maxBounds={[[2.5, 3.5], [15.5, 14.5]]}
        minZoom={5.5}
      >
        {events.map((event, i) => {
          const [lng, lat] = getCoords(event, i)
        ...
        {hoveredEvent && (
          <Popup
            longitude={getCoords(hoveredEvent, events.indexOf(hoveredEvent))[0]}
            latitude={getCoords(hoveredEvent, events.indexOf(hoveredEvent))[1]}
